...
class TrustFlowMiddleware:
    """WSGI middleware that captures request/response details and ships them
    to TrustFlow automatically."""

    def __init__(self, app, api_key=None, endpoint=None):
        """
        Args:
            app:      The WSGI application to wrap.
            api_key:  TrustFlow API key (falls back to env var).
            endpoint: TrustFlow API base URL (falls back to env var).
        """
        self.app = app
        self.tp = TrustFlow(api_key=api_key, endpoint=endpoint)
